<template>
  <div>
    ...
    <div class="min-h-screen bg-base-100 p-6 lg:p-8">
      <!-- Header -->
      <header class="mb-6 w-full max-w-4xl mx-auto">
        <nav class="flex items-center justify-end gap-4">
          <Link v-if="$page.props.auth?.user" :href="dashboard()" class="btn btn-outline btn-sm"> Dashboard </Link>
          <template v-else>
            <Link :href="login()" class="btn btn-ghost btn-sm"> Log in </Link>
            <Link :href="register()" class="btn btn-outline btn-sm"> Register </Link>
          </template>
        </nav>
      </header>

      <!-- Main Content -->
      <div class="flex items-center justify-center lg:min-h-[80vh]">
        <main class="w-full max-w-4xl">
          <div class="card bg-base-100 shadow-xl border border-base-300">
            <div class="card-body p-6 lg:p-20">
              <h1 class="text-2xl font-semibold mb-2">Let's get started</h1>
