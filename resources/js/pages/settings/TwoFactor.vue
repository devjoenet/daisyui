    ...
    <SettingsLayout>
      <div class="space-y-6">
        ...
        <div v-if="!twoFactorEnabled" class="flex flex-col items-start justify-start space-y-4">
          ...
          <div>
            <button v-if="hasSetupData" @click="showSetupModal = true" class="btn btn-primary"><ShieldCheck />Continue Setup</button>
            <Form v-else v-bind="enable.form()" @success="showSetupModal = true" #default="{ processing }">
              <button type="submit" :disabled="processing" class="btn btn-primary"><ShieldCheck />Enable 2FA</button>
            </Form>
          </div>
        </div>
        ...
        <div v-else class="flex flex-col items-start justify-start space-y-4">
          ...
          <div class="relative inline">
            <Form v-bind="disable.form()" #default="{ processing }">
              <button class="btn btn-error" type="submit" :disabled="processing">
                <ShieldBan />
                Disable 2FA
              </button>
            </Form>
