      ...
      <div class="flex flex-col space-y-6">
        ...
        <Form v-bind="ProfileController.update.form()" class="space-y-6" v-slot="{ errors, processing, recentlySuccessful }">
          <div class="grid gap-2">
            ...
            <input id="name" class="input input-bordered w-full mt-1" name="name" :value="user.name" required autocomplete="name" placeholder="Full name" />
            <span v-if="errors.name" class="text-error text-xs mt-2">{{ errors.name }}</span>
          </div>

          <div class="grid gap-2">
            <label for="email" class="block text-sm font-medium text-gray-700">Email address</label>
            <input id="email" type="email" class="input input-bordered w-full mt-1" name="email" :value="user.email" required autocomplete="username" placeholder="Email address" />
          ...
          <div class="flex items-center gap-4">
            <button :disabled="processing" data-test="update-profile-button" class="btn btn-primary">Save</button>
