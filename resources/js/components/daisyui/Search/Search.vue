<script setup lang="ts">
  ...
  const listId = `search-list-${Math.random().toString(36).slice(2, 9)}`;

  const queryValue = computed(() => (query.value === "" ? null : { id: null, name: query.value }));

  const filteredValues = computed(() => {
    let values = props.listValues;
    if (query.value) {
      values = values.filter((el) => el.name.toLowerCase().includes(query.value.toLowerCase()));
    }
    if (props.limit) {
    ...
    if (props.multiple) {
      ...
      if (newQuery.includes(",")) {
        ...
        for (const part of parts) {
          const match = props.listValues.find((o) => o.name.toLowerCase() === part.toLowerCase());
  ...
  function onKeydown(e: KeyboardEvent) {
    const len = filteredValues.value.length + (props.addOption && queryValue.value ? 1 : 0);
...
<template>
  <div ref="root" class="relative" role="combobox" :aria-expanded="open" :aria-owns="listId" @focus="onFocus">
    ...
    <transition enter-active-class="transition ease-out duration-100" enter-from-class="opacity-0 scale-95" enter-to-class="opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-95">
      <ul v-if="open" :id="listId" class="dropdown-content absolute mt-1 max-h-60 w-full overflow-auto rounded-box bg-base-100 shadow z-50 menu flex-nowrap p-2" role="listbox">
        <li
          v-if="props.addOption && queryValue"
          ...
          @mousedown="selectValue(queryValue)"
        >
        ...
        <li
          ...
          role="option"
          :aria-selected="highlightedIndex === (props.addOption && queryValue ? i + 1 : i)"
