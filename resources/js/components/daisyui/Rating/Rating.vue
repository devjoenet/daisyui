...
<template>
  ...
  <div v-if="items && !$slots.default" :class="ratingClass">
    <template v-for="(item, index) in items" :key="index">
      <RatingItem :value="item.value" :checked="internalValue === item.value" :shape="shape" :color="color" :half-mask="halfStar && index % 2 === 0 ? '1' : halfStar && index % 2 === 1 ? '2' : undefined" :disabled="disabled" @change="handleChange" />
    </template>
  </div>
  ...
  <div v-else-if="count > 0 && !$slots.default" :class="ratingClass">
    <template v-for="i in count" :key="i">
      <RatingItem v-if="halfStar" :value="i - 0.5" :checked="internalValue === i - 0.5" :shape="shape" :color="color" half-mask="1" :disabled="disabled" @change="handleChange" />

      <RatingItem :value="i" :checked="internalValue === i" :shape="shape" :color="color" :half-mask="halfStar ? '2' : undefined" :disabled="disabled" @change="handleChange" />
    </template>
  </div>
