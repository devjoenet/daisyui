...
<template>
  <div class="overflow-x-auto">
    <!-- Dynamic mode -->
    <table v-if="columns && rows" :class="['table', sizeClass, zebraClass, pinRowsClass, pinColsClass, customClassValue]">
      <thead v-if="$slots.header || header">
        <template v-if="$slots.header">
          <slot name="header"></slot>
        </template>
        <tr v-else>
          <th v-for="(column, index) in columns" :key="index" :class="column.customClass">
      ...
      <tbody>
        <tr v-for="(row, rowIndex) in rows" :key="row.id" :class="row.customClass">
    ...
    <!-- Manual mode -->
    <table v-else :class="['table', sizeClass, zebraClass, pinRowsClass, pinColsClass, customClassValue]">
      <slot></slot>
    </table>
  </div>
</template>
