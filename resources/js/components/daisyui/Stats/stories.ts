...
<Stats :items="items" :shadow="true">
  ...
  <template #actions="{ item }">
    <Badge v-for="tag in item.tags" :key="tag" size="sm" ghost class="mr-2">{{ tag }}</Badge>
  </template>
</Stats>
`;
