...
<template>
  <div :class="['stats', verticalClass, shadowClass]">
    ...
    <template v-else-if="items">
      <Stat v-for="(item, index) in items" :key="index" :figureClass="item.figureClass" :valueClass="item.valueClass" :descClass="item.descClass" :titleClass="item.titleClass">
        <template v-if="item.figure || $slots.figure" #figure>
          <slot name="figure" :item="item">
            <component :is="item.figure" v-if="typeof item.figure === 'object'" />
            <img v-else-if="typeof item.figure === 'string' && item.figure.startsWith('http')" :src="item.figure" :alt="item.title" />
            <div v-else-if="typeof item.figure === 'string'" v-html="item.figure"></div>
          </slot>
        </template>
        ...
        <template v-if="item.actions || $slots.actions" #actions>
          <slot name="actions" :item="item">
            <component :is="item.actions" v-if="typeof item.actions === 'object'" />
          </slot>
        </template>
      </Stat>
    </template>
