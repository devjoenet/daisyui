...
<template>
  <div class="relative" ref="root" @keydown="onKeydown">
    <div
      class="input input-bordered min-h-10 flex items-center gap-2 cursor-text w-full overflow-x-clip"
        ...
        <template v-for="(val, idx) in selectedValues" :key="valKey(val, idx)">
          <slot name="tag" :value="optionFromValue(val)" :index="idx">
            <label class="badge badge-sm badge-neutral mr-1 flex items-center gap-2 cursor-pointer">
              {{ labelFromValue(val) }}
              <button class="btn btn-ghost btn-circle btn-xs p-0" @click.stop="removeValue(val)" aria-label="Supprimer">✕</button>
            </label>
          </slot>
        </template>
      ...
      <template v-else>
        <template v-if="searchable">
          <input
            ...
            @focus="open = true"
            @input="onQuery"
            class="input input-ghost flex-1 min-w-24 outline-none! pl-0"
    ...
    <transition enter-active-class="transition ease-out duration-100" enter-from-class="opacity-0 scale-95" enter-to-class="opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-95">
      <div v-if="open" class="absolute dropdown-content menu flex-nowrap bg-base-100 rounded-box shadow mt-1 w-full z-50 p-0" role="listbox" :id="listId">
        <!-- optional internal search at top of dropdown -->
        <div v-if="searchable && searchableInside" class="p-2">
          <input v-model="query" ref="searchInputInside" class="input input-bordered w-full" :placeholder="searchPlaceholder" @input="onQuery" />
        </div>
          ...
          <ul class="block m-0 p-2">
            ...
            >
              <a class="flex items-center gap-3 py-2 px-3 text-balance!">
                <input v-if="checkboxes" type="checkbox" class="checkbox checkbox-sm" :checked="isSelected(opt)" @change.stop.prevent="toggleOption(opt)" tabindex="-1" />
