...
<template>
  ...
  <template v-else-if="item.isTitle && item.subItems">
    <template v-if="$slots[`title-${idx}`]">
      <slot :name="`title-${idx}`" :item="item" :index="index" />
      <ul role="listbox">
        <menu-item v-for="(sub, subIndex) in item.subItems" :key="subIndex" :item="sub" :index="subIndex" :parent-index="idx" v-bind="$attrs">
  ...
  <template v-else-if="item.subItems">
    ...
    <template v-else>
      <li>
        <a
          :role="item.disabled ? undefined : 'option'"
          :href="item.href"
          :class="{
  ...
  <template v-else>
    ...
    <template v-else>
      <li :class="{ 'menu-disabled': item.disabled }">
        <a :role="item.disabled ? undefined : 'option'" :href="item.href" :class="{ 'menu-active': isActive }" @click.stop="item.onClick && item.onClick()">
          <!-- Checkbox pour sélection multiple -->
          <input v-if="item.multiple && item.value !== undefined" type="checkbox" class="invisible w-0 h-0 overflow-clip" :checked="item.checked" disabled />
          {{ item.label }}
        </a>
      </li>
    </template>
