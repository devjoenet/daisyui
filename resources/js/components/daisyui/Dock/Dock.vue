...
<template>
  <div :class="['dock', sizeClass, reverseTheme && 'bg-neutral text-neutral-content']">
    <template v-if="$slots.default">
      <slot></slot>
    </template>
    <template v-else-if="items">
      <button v-for="(item, index) in items" :key="index" @click="setActiveItem(index)" :class="[item.class, isActive(index) && 'dock-active']">
        <slot name="icon" :item="item" :index="index">
          <slot :name="`icon-${index}`" :item="item" :index="index">
            ...
            <img v-else-if="typeof item.icon === 'string' && item.icon.startsWith('http')" :src="item.icon" :alt="item.label" />
