<script setup lang="ts">
  ...
  import { type TIMELINEDirection, type TIMELINEModifier, type TIMELINEItem } from "./types";
  ...
  const getLineClass = (index: number): string => {
    ...
    if (props.items && props.items[index] && props.items[index].valid !== undefined) {
  ...
  <ul :class="timelineClasses">
    ...
    <template v-if="items">
      <li v-for="(item, index) in items" :key="index" :class="item.customClass">
        ...
        <div v-if="item.start || $slots[`start-${index}`]" class="timeline-start">
            ...
            <slot name="middle" :item="item" :index="index">
              ...
              <template v-else>
                <svg
                  ...
                  viewBox="0 0 20 20"
                  fill="currentColor"
                  :class="['w-5 h-5', item.valid === true ? 'text-success' : item.valid === false ? 'text-error' : validItems && validItems[index] === true ? 'text-success' : validItems && validItems[index] === false ? 'text-error' : '']"
                >
        ...
        <div v-if="item.end || $slots[`end-${index}`]" class="timeline-end" :class="{ 'timeline-box': modifier === 'timeline-box' || true }">
