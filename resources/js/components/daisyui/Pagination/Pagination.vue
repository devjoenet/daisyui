<script lang="ts" setup>
  ...
  const pages = computed(() => {
    ...
    for (let i = Math.max(2, current - 1); i <= Math.min(current + 1, totalPages.value - 1); i++) {
      result.push(i);
    }
...
<template>
  <Join>
    ...
    <template v-else>
      <button v-if="showFirst" class="join-item btn" :class="[sizeClass, colorClass, outlineClass, { 'btn-disabled': modelValue <= 1 }]" @click="changePage(1)" :disabled="modelValue <= 1">
        <slot name="first">{{ firstLabel }}</slot>
      </button>

      <button v-if="showPrevious" class="join-item btn" :class="[sizeClass, colorClass, outlineClass, { 'btn-disabled': modelValue <= 1 }]" @click="changePage(modelValue - 1)" :disabled="modelValue <= 1">
        <slot name="previous">{{ previousLabel }}</slot>
      </button>

      <button v-for="page in pages" :key="page" class="join-item btn" :class="[sizeClass, colorClass, outlineClass, { 'btn-active': page === modelValue }, { 'btn-disabled': page === '...' }]" @click="changePage(page)">
        <slot :name="`page-${page}`">{{ page }}</slot>
      </button>

      <button v-if="showNext" class="join-item btn" :class="[sizeClass, colorClass, outlineClass, { 'btn-disabled': modelValue >= totalPages }]" @click="changePage(modelValue + 1)" :disabled="modelValue >= totalPages">
        <slot name="next">{{ nextLabel }}</slot>
      </button>

      <button v-if="showLast" class="join-item btn" :class="[sizeClass, colorClass, outlineClass, { 'btn-disabled': modelValue >= totalPages }]" @click="changePage(totalPages)" :disabled="modelValue >= totalPages">
