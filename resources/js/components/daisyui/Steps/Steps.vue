<script setup lang="ts">
  ...
  const getStepClass = (index: number): string[] => {
    ...
    const variantClass = props.variant === "default" ? "" : `step-${props.variant}`;
...
<template>
  <ul :class="stepsClasses">
    <!-- Dynamic items mode -->
    <template v-if="items">
      <li v-for="(item, index) in items" :key="index" :class="getStepClass(index)" :data-content="item.dataContent">
        <slot :name="`step-${index}`" :item="item" :index="index">
          <slot name="step" :item="item" :index="index">
            <slot v-if="item.label">
              {{ item.label }}
            </slot>
            <slot v-if="$slots[`step-icon-${index}`]" :name="`step-icon-${index}`"></slot>
