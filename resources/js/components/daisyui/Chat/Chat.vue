...
<template>
  <!-- Dynamic items mode -->
  <template v-if="items">
    <div v-for="(item, index) in items" :key="index" :class="['chat', getPlacementClass(item.placement), item.customClass || customClass]">
      <div v-if="item.image || $slots[`image-${index}`]" class="chat-image avatar">
      ...
      <div :class="['chat-bubble', item.variant ? `chat-bubble-${item.variant}` : '']">
